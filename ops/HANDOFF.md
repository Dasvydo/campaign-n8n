...
| **P-2** | **Nurture opt-in.** The 1-9 opt-in is a `mailto:`, so consent never reaches n8n and WF-C2 stays manual. ~15 minutes to make it a POST, but it also needs `campaign-ledger/migrations/004_consent.sql`. | Changes the shared contract and depends on a migration only you run. It is also the one place the campaign chose legal caution deliberately. |
...
| **P-3** | **Where `campaign-ledger/migrations/004_consent.sql` lives.** It is a `campaign-ledger` migration sitting in `campaign-n8n` because Batch F may not write to a sibling repo. Marked `NOT RUN. NOT APPLIED.` | Running it is your action. |
